from cm_vispy.cmtabletop.scenes.scene import Scene
...
class SceneBoard(Scene):
    ...
    def __init__(self, cards, half):
        ...
        for a in self.actors:
            a.surface = SceneBoard.im.copy()

    def draw(self, program):
        for abg_i in range(SceneBoard.board_space):
            SceneBoard.im.get_tex_vertices(*self.get_coords(abg_i))
            program['u_texture'] = SceneBoard.im.get_tex()
            program.bind(SceneBoard.im.get_vbo())
    ...
    def _get_image(self, pos):
        ...
        if type(self.actors[pos]) == Card:
            ...
            im_ret = transform.smoothscale(im_ret, (int(Game.card_w), int(Game.card_h)))
        ...
        if flag is not None and clr is not None:
            im_ret.fill(clr, (0, 0, Game.card_w, Game.card_h-Game.card_w*0.5), flag)
        return im_ret
    ...
    def get_coords(self, pos, half=-1):
        if half < 0:
            half = self.half
        ygap = [3, 2, 1, 2, 3]
        marginx = GV.SCR_W * 0.25
        xgap = (GV.SCR_W - marginx * 2 - GV.CARD_W * SceneBoard.board_space) / \
               (SceneBoard.board_space + 1)
        xpos = marginx + xgap * (pos + 1) + GV.CARD_W * pos
        ypos = GV.SCR_H / 2 + (half - 1) * GV.CARD_H
        ypos += GV.CARD_H * (2 * half - 1) * ygap[pos] * 0.1
        return xpos, ypos
