...
class Card(Actor):
    ...
    def __init__(self, fromname):
        ...
        self.im = image.load(Card.path + self.name + '.png').convert_alpha()
        self.im_abl = [image.load(Card.abl_path + i + '.png').convert_alpha() for i in self.abl.keys()]

        self.used = [False for _ in range(len(self.abl.keys()))]
